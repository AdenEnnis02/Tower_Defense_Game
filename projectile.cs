...
public class Projectile {
    ...
    void Update() {
        ...
        if ( !target ) {
            Destroy(gameObject);
            // Write more code to spawn an explosion / splat effect.
        ...
        Vector2 direction = target.position - currentPosition;
        currentPosition += direction.Normalize() * speed * Time.deltaTime;
        ...
        float totalDistance = Vector2.Distance(origin, target.position);
        float heightOffset = arcFactor * totalDistance * Mathf.Sin( distanceTravelled * Mathf.PI / totalDistance );
        transform.position = currentPosition + new Vector2( 0, 0, heightOffset );
    ...
    public static Projectile Spawn(GameObject prefab, Vector2 position, Quaternion rotation, Transform target) {
...
https://blog.terresquall.com/2019/11/coding-projectiles-for-your-tower-defense-game-part-2/